...
namespace MInt
{
    public static unsafe class MStats
    {
        ...
        const long MAX_HEAP_SIZE = MAX_THREAD_HEAP_SIZE * MAX_THREADS * MSPAN_SIZE;
        ...
        public static void EndSpan(ulong mSpanID)
        {
            GetMapThread().EndSpan(mSpanID);
        }

        static MThread GetMapThread()
        {
            if (!_threadID.IsValueCreated) _threadID.Value = Interlocked.Increment(ref _mThreadIdx) - 1;
            return _mThread[_threadID.Value];

            // old (probably much worse) path
        ...
        static MThread AddMapThread()
        {
            ...
            {
                int idx = Interlocked.Increment(ref _mThreadIdx) - 1;
